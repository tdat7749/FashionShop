...
namespace FashionStore.Application.Services.Catalog.SProduct
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ApiResult<bool>> UpdateProduct(UpdateProductRequest request)
        {
            ...
            //Update add option cho product
            foreach (var item in request.productInOptions)
            {
                var abc = listOptionProduct.Where(x => x.ProductId == request.ProductId && x.OptionId == item).FirstOrDefault();
                if (abc != null)
                {
                    continue;
                }
                ...
                {
                    var optionAdd = new ProductInOption();
                    optionAdd.ProductId = request.ProductId;
                    optionAdd.OptionId = item;

                    _context.ProductInOptions.Add(optionAdd);
                }
            }


            // Update remove option cho product
            foreach (var item in listOptionProduct)
            {
                if (request.productInOptions.Contains(item.OptionId))
                {
                    continue;
                }
                else
                {
                    _context.ProductInOptions.Remove(item);
                }
            }
